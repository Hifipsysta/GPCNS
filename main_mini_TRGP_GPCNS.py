...
def train_projected_regime (args, model, device,x,y,optimizer,criterion,memory, task_name, task_name_list, feature_list, task_id, feature_mat, space1=[None, None, None], space2=[None, None, None]):
    model.train()
    r=np.arange(x.size(0))
    np.random.shuffle(r)
    r=torch.LongTensor(r).to(device)
...
def get_space_and_grad(model, device, mat_list, grad_list, threshold, memory, task_name, task_name_list, task_id, feature_list, grad_stack_list, space_hstack_list, importance_list, space_list):
    ...
    print(len(mat_list))
    for i in range(len(mat_list)):
        print ('Layer {} : {}/{}'.format(i+1,space_list[i].shape[1], space_list[i].shape[0]))
    print_log('-'*40, log)
    ...
    for k,ncla in taskcla:
        ...
        if task_id==0:
            ...
            for i in range(len(space_list)):
                ...
                feature_mat.append(projection_operator)
            ...
            for epoch in range(1, args.n_epochs+1):
                ...
                clock0=time.time()
                train_projected_regime(args, model, device,xtrain, ytrain,optimizer,criterion,memory, task_name, task_name_list, feature_list, task_id, feature_mat, space1=space1, space2=space2)
                clock1=time.time()
                            ...
                            if k == 0:
                                ...
                                for m,params in model.named_parameters():                               
                                    if 'scale2' in m:
                                        ...
                                        idx += 1   
                    # print('#'*30, memory[task_test][str(i)]['selected_task'])
...
if __name__ == "__main__":
    # Training parameters
    parser = argparse.ArgumentParser(description='FEGPCNS (TRGP)')
