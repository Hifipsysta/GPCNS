...
def train_projected_regime (args, model,device,x,y,optimizer,criterion,memory, task_name, task_name_list, feature_list, task_id, projection_list, space1=[None, None, None], space2=[None, None, None]):
    model.train()
    r=np.arange(x.size(0))
    np.random.shuffle(r)
    r=torch.LongTensor(r).to(device)
...
def get_space_and_grad(model, device, mat_list, grad_list, threshold, memory, task_name, task_name_list, task_id, feature_list, grad_stack_list, space_hstack_list, importance_list, space_list):
    ...
    if task_name == 'cifar100-0':
        ...
        for i in range(len(mat_list)):
            ...
            space_hstack_list.append(torch.hstack([space_list[i], torch.tensor(feature_list[i]).float().to(device)]))
        ...
        for i in range(len(mat_list)):
            ...
            if Ours:
                ...
                print_log('space shape:{}'.format(feature_list[i].shape), log)
                #delta = np.dot(np.dot(feature_list[i],feature_list[i].transpose()),activation)
                delta = []
                ...
                act_hat -= np.dot(np.dot(feature_list[i],feature_list[i].transpose()),activation)
                ...
                if len(sel_index_from_U) > 0:
                    ...
                    total_U =  np.hstack((feature_list[i], Ui[:,sel_index_from_U] ))
                    feature_list[i] = total_U
                else:
            ...
            space_hstack_list[i] = torch.hstack([space_list[i], torch.tensor(feature_list[i]).float().to(device)])
            U_matrix, Sigma, Vh_matrix = torch.linalg.svd(space_hstack_list[i], full_matrices=False)


            Sigma_total = torch.sum(Sigma**2)
            Sigma_accumul_list = [torch.sum(Sigma[0:idx+1]**2).item() for idx in range(len(Sigma))]
            Sigma_select = torch.tensor(Sigma_accumul_list).to(device) <= threshold[i]*Sigma_total
            Sigma_slim = Sigma[Sigma_select]
            rank_subspace = torch.count_nonzero(Sigma_slim)
    ...
    for i in range(4):
        print_log ('Layer {} : {}/{}'.format(i+1,space_list[i].shape[1], space_list[i].shape[0]), log)
    print_log('-'*40, log)
    
    return feature_list, grad_stack_list, importance_list, space_list  
...
def main(args):
    ...
    for k,ncla in taskcla:
        ...
        if task_id==0:
            ...
            feature_list, grad_stack_list, importance_list, space_list  = get_space_and_grad (model, device, mat_list, grad_list, threshold, memory, task_name, task_name_list, task_id, feature_list, grad_stack_list, space_hstack_list, importance_list, space_list)
            ...
            for epoch in range(1, args.n_epochs+1):
                # Train 
                clock0=time.time()
                train_projected_regime(args, model,device,xtrain, ytrain,optimizer,criterion,memory, task_name, task_name_list,feature_list, task_id, projection_list, space1=space1, space2=space2)
                clock1=time.time()
...
if __name__ == "__main__":
    # Training parameters
    parser = argparse.ArgumentParser(description='FE-GPCNS (TRGP)')
