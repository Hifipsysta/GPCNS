...
class DatasetGen(object):
    ...
    def get(self, task_id):
        ...
        self.dataloaders[task_id]['train'] = {'x': [],'y': [], 'saug': []}
        ...
        for data, label in train_loader:
            ...
            for img in data:
                img = pil_transform(img.squeeze_(0))
                strong_image = crop(img)
                strong_image = augmentations.aug(strong_image, totensor_transforms)
                self.dataloaders[task_id]['train']['saug'].append(strong_image)

            print('#'*100, len(self.dataloaders[task_id]['train']['saug']), type(self.dataloaders[task_id]['train']['saug']))
            print('#'*100, len(self.dataloaders[task_id]['train']['x']),  type(self.dataloaders[task_id]['train']['x']))
