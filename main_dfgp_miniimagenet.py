...
def update_GradientMemory (model, mat_list, grad_list, threshold, feature_list=[], grad_stack_list=[], space_list=[], space_hstack_list=[]):
    ...
    for k,ncla in taskcla:
        ...
        if task_id==0:
            ...
            for epoch in range(1, args.n_epochs+1):
                ...
                print(' Valid: loss={:.3f}, acc={:5.1f}% |'.format(valid_loss, valid_acc))
                # Adapt lr
            ...
            # Memory Update  
            mat_list, grad_list = get_representation_matrix_ResNet18 (model, device, xtrain, ytrain)
            feature_list = update_GradientMemory (model, mat_list, grad_list, threshold, feature_list, grad_stack_list, space_list, space_hstack_list)
        else:
            base_optimizer = optim.SGD(model.parameters(), lr=lr)
            optimizer = SAM(base_optimizer, model)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--mixup_alpha', type=float, default=20, metavar='Alpha',
                        help='mixup_alpha')
    parser.add_argument('--mixup_weight', type=float, default=0.01, metavar='Weight',
                        help='mixup_weight')
